...
namespace Cardio.SPCL
{
    public static class ExtractData
    {
        ...
        /// 接收数据的起始位
        /// </summary>
        public static byte startBitDataReceive = 0xEE;
        public static byte HKstartBitDataReceive = 0xF0;
        /// <summary>
        /// 接收数据的停止位
        /// </summary>
        public static byte endBitDataReceive = 0xFF;

        /// <summary>
        /// 命令字类的对象
        /// </summary>
        private static CommandWord CommandWord = new CommandWord();

        /// <summary>
        /// CRC校验的对象
        /// </summary>
        private static CRCcheck crcCheck = new CRCcheck();


        //ceshi
        public delegate void FunShow(List<byte> data);
        public static FunShow FunShowEvent;
        ...
        private static List<int> FindStartbitIndex(List<byte> receivedData)
        {
            ...
            if (receivedData.Count > 4)  //长度不小于5找起始位才有意义
            {
                ...
                for (int i = 0; i < receivedData.Count - 4; i++)
                {
                    if (receivedData[i] == startBitDataReceive)   //某一位等于起始位
                    {
                        ...
                        indexNum.Add(i);  //是起始位则加入到起始位  索引   号数组中
                    }
                }
            }
            return indexNum;
        ...
        private static List<int> FindEndbitIndex(List<byte> receivedData)
        {
        ...
        public static string ByteToString(List<byte> b)
        {
        ...
        public static List<ReceiveDataStructure> ExtractToDataStructure(List<byte> dataTemp)
        {
            if (dataTemp.Count == 0)
            {
                return null;
        ...
        /// <returns></returns>
        public static List<ReceiveDataStructure> GetUsefullInfos(List<byte> readBuffer)
        {
            List<List<ReceiveDataStructure>> dataValeAndTypes = new List<List<ReceiveDataStructure>>();
                    ...
                    for (int i = 0; i < endBitIndex.Count; i++)    //有几帧数据就读取几帧出来
                    {
                        for (int j = 0; j < startBitIndex.Count; j++)     //找一帧数据出来
                        {
                            if (startBitIndex[j] + readBuffer[startBitIndex[j] + 1] == endBitIndex[i] + 2
                                && readBuffer[startBitIndex[j] + 1] >= 5)  //起始位加上数据长度
                            {
        ...
        /// <param name="readBuffer"></param>
        /// <returns></returns>
        public static List<ReceiveDataStructure> GetHKInfo(List<byte> readBuffer)
        {
            List<ReceiveDataStructure> dataValeAndType = new List<ReceiveDataStructure>();
            if (readBuffer.Count >= 3)  //缓存长度大于5开始判断
            {
                ...
                if (startBitIndex.Count >= 1)  //数据中包含起始位和结束位  
                {
                    ...
                    for (int j = 0; j < startBitIndex.Count; j++)     //找一帧数据出来
                    {
                        ...
                        {
                            if ((startBitIndex[j] + readBuffer[readRufferIndex]) <= (readBuffer.Count - 2))
                            {
                                dataTempNoCRC.Clear();
        ...
        /// <param name="receivedData">接收到的数据数组</param>
        /// <returns>返回起始位索引号组成的数组</returns>
        private static List<int> FindHKStartbitIndex(List<byte> receivedData)
        {
            List<int> indexNum = new List<int>();  //存储索引号的数组
        ...
        /// <param name="dataTemp">去掉CRC位的一帧数据</param>
        /// <returns></returns>
        private static List<ReceiveDataStructure> HKExtractToDataStructure(List<byte> dataTemp)
        {
            if (dataTemp.Count == 0)
            {
                return null;
