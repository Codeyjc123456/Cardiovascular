...
namespace Cardio.Views.MeasurePage
{
    ...
    public partial class LoginPage : Page
    {
        ...
        private async void LoginAPI_ZJ()
        {
            ...
            var response = ApiBLL.DoGetUser_ZJ(url, data);
            ...
            catch { Growl.Warning("获取用户信息失败!"); }
        }
        private async void LoginAPI_BX()
        {
            string url = APPSettingUtil.APP_ApiUrlLogin;
            ...
            string response = await ApiBLL.DoPostUserAsync_BX(bx, loginViewModel.UserID);
            var response_json = (JObject)JsonConvert.DeserializeObject(response);
            UserInfoEntity userInfo = new UserInfoEntity();
            if ((string)response_json["code"] == "200")
            {
                // 方式1：使用 JObject 动态解析
                JObject obj = JObject.Parse(response);
                userInfo?.UserHeight = (double)obj["data"]["user_height"];
                userInfo?.UserWeight = (double)obj["data"]["user_weight"];
                userInfo.UserName = obj["data"]["user_name"].ToString();
                userInfo.UserId = obj["data"]["user_id"].ToString();
                userInfo.UserSex = obj["data"]["user_sex"].ToString();
                userInfo.UserBirthday = obj["data"]["user_birthday"].ToString();
                userInfo.UserAge = Convert.ToInt16(obj["data"]["user_age"]);
                TestMode.AI_Select = true;
                TestMode.LBBP_Select = true;
                TestMode.LABP_Select = true;
                TestMode.RBBP_Select = true;
            ...
            {
                HandyControl.Controls.Growl.Warning($"登录失败:{response_json["message"]}!");
            }
        }
